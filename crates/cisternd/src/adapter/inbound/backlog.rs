...
mod tests {
    ...
    fn registering_with(force: Value) -> Value {
    ...
    fn a_force_that_is_a_boolean_reaches_the_core_as_one() {
        ...
        data(answered(
            &core,
            "task_add",
            registering_with(Value::Bool(true)),
        ));
        ...
        data(answered(
            &core,
            "task_add",
            registering_with(Value::Bool(false)),
        ));
    ...
    fn a_force_nobody_gave_reaches_the_core_as_false() {
        ...
        let core = Core::default();
        data(answered(&core, "task_add", registering_with(Value::Null)));
        assert_eq!(core.forced.get(), Some(false));
    }
    ...
    fn a_force_that_is_not_a_boolean_never_reaches_the_core() {
        for value in [
            ...
            let core = Core::default();
            let refused = failure(answered(&core, "task_add", registering_with(value.clone())));

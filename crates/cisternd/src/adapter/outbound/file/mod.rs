...
//!
//! One file each, read and written whole. `kept` holds what all of them do with
//! a file; each of the rest holds its own format and its own fields.
//!
...
/// A number where the text is one, so that the file reads as JSON rather than
/// as strings holding digits.

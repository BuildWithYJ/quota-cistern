...
//!
//! Three stores keep one file each and treat it the same way.
//! Read the whole of it, hand it over, and put the whole of it back.
//! What they do not share is the format and the fields, which stay with each of them.
...
///
/// The directories are arguments rather than reads.
/// That way, which one is used can be tested without setting a variable the whole process sees.
pub struct Kept {
    path: PathBuf,
    /// Held from a read to the write that follows it.
    /// That way, two writers do not each put back what the other never saw.
    writing: Mutex<()>,
}
...
impl Kept {
    ...
    /// The XDG specification keeps state for what is not worth carrying between machines.
    /// What a user registered is not that.
    pub fn in_data_home(
    ...
    ///
    /// A file nobody has written is not a failure.
    /// Each store says for itself what nothing means, since each has to answer before anything is written.
    ...
    ///
    /// A write that stops halfway leaves the half-written file beside the real one.
    /// The real one is whatever it was.
    /// Renaming is the one step that either happened or did not.
    ...
    ///
    /// A thread that panicked while holding this left the file alone.
    /// The write is the last thing that happens under it.
...
///
/// Beside the real file rather than in a temporary directory.
/// That way the rename stays on one filesystem and cannot become a copy.

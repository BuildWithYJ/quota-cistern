...
| `pacing`      | `holds` · `any`                         | 그 모델의 실행을 지금 속도로는 예산이 못 버틸 때 그 작업을 시작할지. `holds`는 기다리게 두고 `any`는 시작한다. 둘 다 도는 실행을 끊지 않는다 — 예산을 다 썼을 때 돌고 있는 실행을 어떻게 할지는 `locking`이다. 이 값이 정하는 것은 거의 다 쓴 예산 앞에 실행을 몇 개 세워 두느냐이고, 하나하나가 거기서 끊길 실행이다. 기본값은 `any` |

...
`review ls`가 처분을 기다리는 작업을 나열하고 `apply`와 `discard`와 `retry`가 처분하며, 어느
것도 브랜치를 바꾸지 않는다. `tidy`는 처분이 끝난 작업의 작업 공간을 치운다.
...
작업마다 체크아웃이 하나씩 만들어지고 지우는 것이 없어서 계속 쌓인다. 사람이 다 본 것만 치운다. 작업이 끝났고 `apply`나 `discard`로 리뷰 큐에서 빠진 것이 대상이다. 아직 도는 작업은 그 안에 실행이 있어서 남기고, 처분되지 않은 작업은 사람이 결과를 열어보는 자리이자 `retry`가 이어가는 자리라서 남긴다.

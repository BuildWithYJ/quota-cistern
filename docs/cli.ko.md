...
모든 작업의 모든 실행을 `$XDG_DATA_HOME/cistern/runs.jsonl`에 한 줄씩 덧붙이며, 이미 적힌 줄은 고치지 않는다. 각 줄에는 세션이 그 실행 몫으로 잡아 둔 값도 선언한 단위로 함께 남으므로, 무엇을 보고 정했는지가 무엇이 나왔는지 옆에 놓인다. 그 값은 실행이 도는 동안 세션이 자기 예산에서 붙들고 있던 몫이지 실행이 지켜야 했던 한도가 아니다. 벤더가 종료에 대해 무슨 말을 했으면 그 말도 남고, 벤더가 턴을 셌으면 몇 턴이었는지도 남는다. 작업 자신의 사유를 대신하는 것이 아니라 그 옆에 둔다 — 상한에 걸린 작업은 어느 상한이었든 한 낱말을 갖는데, 턴에 걸린 실행과 쓸 수 있는 값에 걸린 실행은 그 작업에 대해 다른 것을 말하기 때문이다. 몫으로 선언한 세션의 실행은 시작 시점과 종료 시점의 사용량 판독값도 함께 남긴다. 그 차이가 그 실행이 선언한 단위로 얼마를 썼는지다. 둘 다 세션이 이미 읽어 둔 값이므로 벤더에게 더 묻지 않는다. 벤더가 거절하면 한 작업이 여러 번 실행되는데 백로그는 가장 최근 것만 들고 있고 `task show`가 보여 주는 것도 그것이다. 예산을 계산하는 값은 이 기록에서 읽으므로 나중 실행이 앞 실행을 덮지 않는다. 이 파일은 저절로 지워지지 않는다.

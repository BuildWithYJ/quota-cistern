...
| `pacing`      | `holds` · `any`                         | 그 모델의 실행을 지금 속도로는 예산이 못 버틸 때 그 작업을 어떻게 할지. `holds`는 미룬다 — 예산이 바닥날 때 돌고 있던 실행은 끊기고 마지막 커밋 이후에 한 일이 사라지며, 넷이 그렇게 끝나면 같은 값을 쓰고 결과 넷을 잃는다. `any`는 시작하고 예산이 끝나는 데서 끝나게 둔다. 기본값은 `any` |
| `locking`     | `cuts` · `waits`                        | 선언한 값을 다 쓴 뒤에도 돌고 있는 실행을 어떻게 할지. `cuts`는 끊는다 — 선언한 값이 넘지 않는 선이 된다. `waits`는 끝내게 둔다 — 한 일은 하나도 잃지 않고, 남은 만큼 선언을 넘겨 쓴다. 기본값은 `waits` |

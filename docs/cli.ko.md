...
번호 대신 지시를 입력하면 그 지시를 대신 등록하며, 처음 것과 같은 방식으로 읽는다. 어느 쪽이든 처음 입력한 문장은 작업의 `original`로 남는다. 실행에 주는 것이 그게 아니기 때문이다. 빈 답은 아무것도 등록하지 않고 1로 끝난다. 터미널에 아무도 없으면 선택지만 출력하고 아무것도 등록하지 않으므로, 스크립트 안의 명령도 무엇을 물으려 했는지 남긴다.

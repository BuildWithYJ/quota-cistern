...
`--version`을 제외한 모든 명령은 저장된 것을 관리하는 코어에 요청한다. 실행 중인 코어가 없으면 명령이 코어를 실행하고 하려던 일을 이어가며, 그 코어는 명령이 끝난 뒤에도 계속 실행된다. `--version`은 양쪽 버전이 맞는지 보고하는 명령이므로 코어가 실행 중이 아니라는 사실이 곧 답이다.

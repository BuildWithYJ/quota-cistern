...
버전이 같아도 빌드가 다를 수 있다. 버전 문자열이 빌드를 담지 않기 때문이다. `--version`은 알 수 있는 한 그것을 말한다. 소켓은 코어가 뜰 때 만들어지므로, 그 소켓보다 나중에 쓰인 코어 프로그램은 어떤 코어도 실행한 적이 없는 것이다. 실패로 다루지 않고 stderr에 한 줄을 적으며 종료 코드는 0이다. 코어를 다시 띄우면 없어진다.
...
`task ceiling`은 실행 하나가 지켜야 하는 상한에 닿아 멈춘 것으로, 세션은 계속된다. 그 상한은 세션 예산의 몫이 아니라 실행 하나가 헛도는 것을 막는 가드이며, 세션이 무엇을 알게 되든 움직이지 않는다. 사용자가 직접 지정하지는 않고 벤더 정의가 들고 있다.
...
자율 실행 중에는 물어볼 사람이 없으므로, 지시가 말하지 않은 것은 전부 에이전트가 스스로 정하게 된다. 작업을 등록하기 전에 지시를 여섯 칸짜리 명세로 풀어내고, 그 명세가 아직 남겨 둔 결정을 센다.
...
`--force`는 이 과정 전부를 건너뛰고 적힌 그대로 등록한다.
...
| 필드            | 타입     | 설명               |
| ------------- | ------ | ---------------- |
| `outcome`     | enum   | `registered`, 또는 아무것도 등록하지 않았으면 `unconfirmed` |
...
| `parts`       | object 배열 | 명세. 칸마다 `part`, `said`, `settled`(`given`/`inferred`/`open`), `drawn_from`, `others`. `unconfirmed`일 때만 |
| `undecided`   | object 배열 | 에이전트에게 남은 결정. `part`와 `decides`. `unconfirmed`일 때만 |
...
| --- | --------------------------- |
| 0   | 성공                          |
| 1   | 아무것도 등록하지 않음: 명세가 아직 에이전트에게 결정을 남겨 뒀는데 정해 줄 사람이 없음. `--force`는 적힌 그대로 등록한다 |
| 2   | 인자 오류 (예: `--title` 누락)     |
...
번호를 입력하면 그 줄을 고치고, `n`을 입력하거나 어느 질문에서든 빈 줄을 넣으면 아무것도 등록하지 않고 1로 끝난다. 처음 입력한 한 줄은 작업의 `original`로 남는다. 등록되는 것은 그 줄이 아니라 명세이기 때문이다. 터미널에 아무도 없으면 명세만 출력하고 아무것도 등록하지 않으므로, 스크립트 안의 명령도 무엇을 물으려 했는지 남긴다.
...
| `conversation` | string | 마지막 실행이 하던 대화. 이어갈 수 없는 작업은 null                                        |
...
모든 작업의 모든 실행을 `$XDG_DATA_HOME/cistern/runs.jsonl`에 한 줄씩 덧붙이며, 이미 적힌 줄은 고치지 않는다. 각 줄에는 세션이 그 실행 몫으로 잡아 둔 값도 선언한 단위로 함께 남으므로, 무엇을 보고 정했는지가 무엇이 나왔는지 옆에 놓인다. 그 값은 실행이 도는 동안 세션이 자기 예산에서 붙들고 있던 몫이지 실행이 지켜야 했던 한도가 아니다. 그 옆에 따로, 벤더가 종료에 대해 무슨 말을 했으면 그 말이 남고 벤더가 턴을 셌으면 몇 턴이었는지가 남는다. 작업 자신의 사유를 대신하는 것이 아니라 그 옆에 둔다 — 상한에 걸린 작업은 어느 상한이었든 한 낱말을 갖는데, 턴에 걸린 실행과 쓸 수 있는 값에 걸린 실행은 그 작업에 대해 다른 것을 말하기 때문이다. 몫으로 선언한 세션의 실행은 한도 판독 둘도 함께 남기며 그 차이가 그 실행에 선언한 단위로 매겨진 값이다. 둘은 그 실행이 끝나기 전에 세션이 마지막으로 읽은 값과 그 실행이 끝날 때 읽은 값이다. 실행이 시작할 때와 끝날 때 잰 값이 아니라 실행 양옆에 그은 회계상의 선이다. 둘 다 세션이 이미 읽어 둔 값이므로 벤더에게 더 묻지 않는다. 뒤쪽 판독은 언제 읽었는지도 함께 남기는데 그것은 실행이 끝난 시각이 아니다. 한도를 읽으려면 세션을 하나 띄워 상태줄을 기다려야 하고 그동안 한도를 움직인 것은 무엇이든 그 값에 들어가기 때문이다. 실행이 출발한 판독은 그 실행이 끝나기 전에 세션이 마지막으로 읽은 값이다. 한 세션의 작업들은 나란히 돌므로 먼저 시작한 실행이 아니라 먼저 끝난 실행의 판독이다. 그 시각은 그 줄에 있으므로 여기서 되풀이하지 않는다. 판독 둘 사이에 세션이 아무것도 돌리지 않았다면 그 구간의 이동은 다른 누군가의 것이다. 벤더가 거절하면 한 작업이 여러 번 실행되는데 백로그는 가장 최근 것만 들고 있고 `task show`가 보여 주는 것도 그것이다. 예산을 계산하는 값은 이 기록에서 읽으므로 나중 실행이 앞 실행을 덮지 않는다. 이 파일은 저절로 지워지지 않는다.

세션은 아무것도 끝나지 않아도 선언한 시간에서 멈춘다. 세션에 대해 묻는 것은 작업이 끝날 때뿐이라, 긴 실행 하나가 도는 세션은 그러지 않으면 마감을 모르는 채 넘긴다.
...
`review ls`가 처분을 기다리는 작업을 나열하고 `apply`와 `discard`와 `retry`와 `resume`이
처분하며, 어느 것도 브랜치를 바꾸지 않는다. `tidy`는 처분이 끝난 작업의 작업 공간을 치운다.
...
일을 처음부터 다시 한다. 마지막 실행이 하던 대화는 놓아주고 다음 실행이 새 대화를 시작한다.
`resume`이 그 하나만 다른 같은 명령이다.
...
| `carries_on` | boolean | 다음 실행이 대화를 이어가는지, 새로 시작하는지 |
...
끝난 작업을 백로그로 되돌리되, 마지막 실행이 하던 대화를 이어간다.

```
cistern resume <작업>
```

`retry`와 같되 일을 처음부터 하는 것이 아니라 이어서 한다. 잘린 실행은 셋을 남긴다 — 브랜치에
커밋한 것, 작업 공간에 커밋하지 않은 것, 그리고 하던 대화. `retry`는 앞의 둘을 지키고 이 명령은
셋을 다 지키므로, 다음 실행이 모든 것을 다시 읽는 대신 그 대화를 이어받는다.

작업은 그 대화를 실행이 완주하거나 사람이 결과를 처분할 때까지 들고 있는다. 마지막 실행이
완주한 작업에는 이어갈 대화가 없고, 그런 작업에 이 명령을 쓰면 대화를 새로 시작하므로 `retry`와
같아진다. 답에 대화를 싣지 않는 벤더의 작업도 마찬가지다.

**출력**과 **종료 코드**는 `retry`의 것과 같다.

#### `cistern tidy`

처분이 끝난 작업의 작업 공간을 치운다.

```
cistern tidy
```

작업마다 체크아웃이 하나씩 만들어지고 지우는 것이 없어서 계속 쌓인다. 사람이 다 본 것만 치운다. 작업이 끝났고 `apply`나 `discard`로 리뷰 큐에서 빠진 것이 대상이다. 아직 도는 작업은 그 안에 실행이 있어서 남기고, 처분되지 않은 작업은 사람이 결과를 열어보는 자리이자 다시 대기하는 작업이 이어가는 자리라서 남긴다.

작업 공간이 없어져도 branch는 남으므로 실행이 커밋한 것은 하나도 사라지지 않고 `apply`도 branch에서 읽는다. 커밋하지 않은 변경이 있는 작업 공간은 그대로 둔다. git이 지우기를 거절하고, 여기서 강제로 지우지 않으며, git이 한 말을 그대로 알린다. 사람이 손으로 지워 등록만 남은 것도 이때 함께 정리한다.

**출력**

| 필드 | 타입 | 설명 |
| --- | --- | --- |
| `items` | array | 작업마다 `task`·`worktree`·`kept` |
| `items[].kept` | string | 그대로 둔 까닭. 치운 것은 null |
...
| `pacing`      | `holds` · `any`                         | 그 모델의 실행을 지금 속도로는 예산이 못 버틸 때 그 작업을 시작할지. `holds`는 기다리게 두고 `any`는 시작한다. 둘 다 도는 실행을 끊지 않는다 — 예산을 다 썼을 때 돌고 있는 실행을 어떻게 할지는 `locking`이다. 이미 도는 것이 있을 때만 묻는다 — 아무것도 안 도는 세션은 속도가 어떻든 하나를 시작한다. 예산에서 끊긴 실행도 커밋한 것은 남기므로, 하나를 미루면 예산은 안 쓰고 한 일도 없이 끝난다. 이 값이 정하는 것은 거의 다 쓴 예산 앞에 실행을 몇 개 세워 두느냐이고, 하나하나가 거기서 끊길 실행이다. 기본값은 `holds` |
| `locking`     | `cuts` · `waits`                        | 선언한 값을 다 쓴 뒤에도 돌고 있는 실행을 어떻게 할지. `cuts`는 끊는다 — 선언한 값이 넘지 않는 선이 되고, 끊긴 실행은 커밋한 것을 남기고 그 뒤에 한 일을 잃는다. `waits`는 끝내게 둔다 — 한 일은 하나도 잃지 않고, 남은 만큼 선언을 넘겨 쓴다. 기본값은 `cuts` |
| `timing`      | `fits` · `any`                          | 그 모델의 실행이 세션에 남은 시간보다 오래 걸려 온 작업을 어떻게 할지. `fits`는 끝을 보지 못할 작업을 시작하지 않고 미룬다. `any`는 그냥 시작한다. 둘 다 도는 실행을 끊지 않는다 — 마감을 넘긴 세션은 새로 맡지 않고 도는 것을 끝내게 둔다. 기본값은 `fits` |

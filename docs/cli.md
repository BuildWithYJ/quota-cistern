...
| `pacing` | `holds` · `any` | Whether a session starts a task a run of whose model the budget will not outlast at the rate it is going. `holds` keeps it waiting; `any` starts it. Neither ends a run: what happens to one still going when the budget is spent is `locking`. What this decides is how many runs are in front of a budget that is nearly finished, since each of them is one more to be ended there. Defaults to `any` |
